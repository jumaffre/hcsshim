...
// creates two sandboxes and starts two containers within them. The first sandbox then starts a NFS server and
// the second container tries to mount that sandbox.
func Test_Container_NFSMount_LCOW(t *testing.T) {
	requireFeatures(t, featureLCOW)

	pullRequiredLCOWImages(t, []string{imageLcowK8sPause, imageLcowAlpine, imageLcowUbuntu})
	...
	// container has all NFS client modules required to successfully mount a NFS (this also means that the
	// kernel was correctly built with the NFS client options). `retry=0` ensures it fails immediately
	// instead of retrying. If the kernel isn't correctly configured the call would fail with "No Device"
	// error.
